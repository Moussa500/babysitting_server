...
function getGroupContents(expression, contentsStartPos) {
  const token6 = /\\?./gsu;
  token6.lastIndex = contentsStartPos;
  let contentsEndPos = expression.length;
  let numCharClassesOpen = 0;
  let numGroupsOpen = 1;
  let match;
  while (match = token6.exec(expression)) {
    const [m] = match;
    if (m === "[") {
      numCharClassesOpen++;
...
function preprocess(template, substitutions, preprocessor) {
  let newTemplate = { raw: [] };
  ...
  template.raw.forEach((raw, i) => {
    ...
    if (i < template.raw.length - 1) {
      ...
      if (substitution instanceof Pattern) {
        const result2 = preprocessor(substitution, { ...runningContext, lastPos: 0 });
...
| \\?.
`.replace(/\s+/g, ""), "gsu");
function flagXPreprocessor(value, runningContext) {
  value = String(value);
  ...
  for (const [m] of value.matchAll(token2)) {
    if (ignoringComment) {
      if (m === "\n") {
...
// src/atomic.js
var token3 = new RegExp(String.raw`(?<noncapturingStart>${noncapturingDelim})|(?<capturingStart>\((?:\?<[^>]+>)?)|\\?.`, "gsu");
function atomicPlugin(expression, data) {
  ...
  do {
    ...
    let inAG = false;
    let match;
    token3.lastIndex = Number.isNaN(aGPos) ? 0 : aGPos + emulatedAGDelim.length;
    while (match = token3.exec(expression)) {
...
var baseToken = new RegExp(String.raw`
\\(?: \d+
  | c[A-Za-z]
  | [gk]<[^>]+>
  | [pPu]\{[^\}]+\}
...
var token5 = new RegExp(String.raw`
\[\^?-?
| --?\]
| (?<dp>[${doublePunctuatorChars}])\k<dp>
| --
...
function backcompatPlugin(expression) {
  ...
  let result = "";
  for (const { 0: m, groups: { dp, vOnlyEscape } } of expression.matchAll(token5)) {
    if (m[0] === "[") {
      if (inCharClass) {
...
var WrappedRegex = class _WrappedRegex extends RegExp {
  #captureMap;
  /**
  ...
  */
  constructor(expression, flags, data) {
    ...
    } else if (expression instanceof _WrappedRegex) {
